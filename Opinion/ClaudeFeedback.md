...
## 2026-05-23: 🔴🔴 JSJ048 バグ再発 (6 日連続) + 根本治療 (T1)

### 発覚
ユーザーが「いま週次レポート送るんだっけ」と確認 → 念のため snapshot 累計
確認したら **126 件 = 5/16 当日分のみ**。5/17〜5/22 の 6 日間蓄積ゼロ。

### 原因
5/16 修正 (JSJ048 retry 12 回 × 300 秒 = 60 分) では不足。5/17〜5/22 は
60 分超でも JSJ048 が前日返した日があり、lines cron も daemon も諦めて exit。

### 根本治療 (T1, 採用)
JSJ048 を完全撤廃して、`ingest_today_lines.py` を `ingest_yesterday.py` と
同じパターンに書き換え:
- 全 43 場 raceprogram (PC0201) を明示日付で直叩き
- 開催無しは PC0201 空で 0.8 秒抜け
- 全 43 場ループで **約 6 秒で完走**
- JSJ048 経由ゼロ → 「前日返し」問題が構造的に発生しない

### 5/23 6:57 動作確認
- 当日 8 場開催 / 35 場 no_race / errors 0 / 6 秒で完走 ✅
- entries 574 行、lines 449 行取得済
- 8:00 daemon 起動で snapshot 自動収集 (現時点 6:57 で間に合う)

### 累計被害 (5/13〜5/22、10 日分)
| データ | 状態 |
|---|---|
| entries/stats/results 5/13〜22 | ✅ 完備 (results cron 救済) |
| ML 本線データ | ✅ 完全無傷 |
| race_lines narabi 5/13〜15, 5/17〜22 (9 日) | ❌ 永久ロスト (低価値、影響軽微) |
| pre-start snapshot 5/13〜15, 5/17〜22 (9 日) | ❌ 永久ロスト (Phase 6 実害) |

### 教訓
- **二度目の同パターンバグ**: 5/16 で発見、修正失敗、5/23 で再発見
- retry で粘る対症療法は脆い → 外部依存 (JSJ048 today list) は構造的に排除すべきだった
- **週次 drift report を未稼働にしていたのが致命的**: 蓄積 0 を 6 日見逃した
